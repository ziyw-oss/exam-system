...
def infer_year_and_paper_type_from_path(pdf_path: str):
    filename = Path(pdf_path).stem  # '704148-question-paper-algorithms-and-problem-solving'
    
    # 提取年份（4位数字，2000以后的）
    year_match = re.search(r'20\d{2}', pdf_path)
    year = int(year_match.group()) if year_match else 2023  # 默认值

    # 提取 paper_type slug
    slug_match = re.search(r'question-paper-(.*)', filename)
    paper_type_slug = slug_match.group(1) if slug_match else "unknown-paper-type"

    # 转为标题格式
    paper_type = paper_type_slug.replace("-", " ").title()
...
if __name__ == "__main__":
    ...
    print(f"📘 推断科目: {paper_type}")
    save_json(parsed, "parsed_debug.json")
 ...
 #################extract_all_code_blocks####################
    ...
    # 3. 匹配代码块，修改 structured 内容
    structured = match_code_blocks_to_questions(structured, code_blocks)

    # 4. 保存结果（可选）
    with open("structured_with_code_blocks.json", "w") as f:
